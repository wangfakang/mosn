...
import (
	...
	"mosn.io/pkg/networkextention/l7/discovery"
	"mosn.io/pkg/utils"
)

func init() {
	if l, err := NewMonitorDiscovery("antvip"); err == nil {
		l.start()
	} else {
...
const (
	GetAntNameSrvLoopInterval       = 3 * time.Second
	GetAntNameSrvLoopInterval4RETRY = 1 * time.Second
	maxTimes                        = 5
)
...
// some information between zone and IDC etc.
type zoneInfoMapIdc struct {
	Gslb []struct {
		IdcName string `json:"idcName"`
		Zones   []struct {
			Elastic      bool   `json:"elastic"`
			Ldr          string `json:"ldr"`
			PercentEnd   int    `json:"percentEnd"`
			PercentIs    bool   `json:"percentIs"`
			PercentStart int    `json:"percentStart"`
			Rdr          string `json:"rdr"`
...
type MonitorDiscovery struct {
	...
	discovery discovey.Discovery
}

func NewMonitorDiscovery(typ string) (*MonitorDiscovery, error) {
	md := &MonitorDiscovery{}

	if disy, err := discovery.GetDiscovery(typ); err != nil {
		return nil, err
	} else {
		md.discovery = disy
		return md, nil
...
func (l *MonitorDiscovery) start() {
	...
	utils.GoWithRecover(func() {
		...
		hostList := make([]string, 0, 8)
		times := 0
		updateSuc := false
		duration := int(GetAntNameSrvLoopInterval)
		idcIndex := make(map[int]string)
			...
			for _, service := range services {
				if changed := l.discovery.CheckAndResetServiceChange(service); changed {
					...
					for _, si := range servicesAddrInfo {
						endpointer.AddEndpoint(si.GetServiceIP(), si.GetServiceIDC(), si.GetServicePort())
					}
					...
					if cs := endpointer.GenerateHackCluster(service); len(cs) != 0 {
						if err := notifyEnvoyAddorUpdateCluster(cs); err != nil {
							updateSuc = false
							times = 1
							duration = int(GetAntNameSrvLoopInterval4RETRY)
							l.discovery.SetServiceChange(true)
...
func (l *vipTransferEndpoints) AddEndpoint(ip, idc string, port int) {
	var endpoint hostAndMetadata
	endpoint.Endpoint.Address.SocketAddress.Address = ip
	endpoint.Endpoint.Address.SocketAddress.PortValue = port
...
func (l *vipTransferEndpoints) GenerateHackCluster(serviceName strng) string {
	endpoints, _ := l.GetEndpointsbyJsonString()
	if len(endpoints) != 0 {
		return subsetClusterStart + endpoints + subsetClusterEnd
	} else {
		return ""
	}
}
